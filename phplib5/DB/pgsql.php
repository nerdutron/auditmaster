...
class DB_pgsql extends DB_common
{
    ...
    // {{{ insertId()
    /**
    * return the value of the last sequence number for the given table
    *
    * @return mixed DB_Error or message
    *
    * @access public
    */
    function insertId($seq_name)
    {
        $seqname = $this->getSequenceName($seq_name);
        if ($this->affectedRows() > 0) {
            $result = $this->getOne("SELECT currval('${seqname}')");
            if (DB::isError($result)) {
                    return $this->raiseError($result);
            }
            return $result;
        } else {
            return $this->errorNative();
        }
    }
